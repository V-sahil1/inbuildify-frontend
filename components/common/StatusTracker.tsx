    ...
    <div className="p-3 bg-card-color">
      <div className="mt-10 flex flex-col gap-[20px] relative">
        ...
        {stages.map((stage, index) => (
          <div
            key={stage.id}
            className={`z-[2] bg-body-color relative rounded-xl border transition-shadow ${stage.status === 'active' ? 'shadow-lg' : 'shadow-sm'
              }`}
          >
            <div
              className={`flex items-center justify-between px-3 py-1 w-full h-16 ${stage.status === 'disabled' ? 'opacity-50 pointer-events-none' : ''
                }`}
            >
