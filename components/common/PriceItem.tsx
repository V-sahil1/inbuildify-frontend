      ...
      >
        ...
        <div className="table-cell p-3 align-top">
          ...
          <div className="font-medium text-[16px] break-all">
            <Tooltip title={item.shortDescription ? item.shortDescription : item.description} > <p className="flex items-center"><p className="line-clamp-2">{item.shortDescription ? item.shortDescription : item.description}</p>{isEditOpen && <IconPencil size={15} className="ml-2 text-blue" />}</p></Tooltip>
          </div>
          {isEditOpen && <div className="flex gap-1 items-center text-blue text-sm mt-1"><div className="rounded-full w-4 h-4 text-blue flex items-center justify-center bg-red-50" onClick={() => setIsNotesOpen(true)}><IconPlus size={12} /></div><p className="">Notes</p></div>}
          {isNotesOpen &&
            <div className="bg-card-color p-3 ">
              <TextArea showCount maxLength={500} />
              <div className="flex gap-2 justify-end mt-6">
                <Button size="small" onClick={() => setIsNotesOpen(false)}>Cancel</Button>
                <Button size="small" type="primary" onClick={handleNotesEdit}> OK</Button>
                {isEditNotesopen && <Button type="primary" onClick={handleNotesDelete}>Delete</Button>}
              </div>
            </div>}
        </div>
        {/* Quantity */}
