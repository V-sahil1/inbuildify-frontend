...
const JobVariationCreateTable: React.FC<JobVariationCreateTableProps> = ({
  dataSource,
  setDataSource,
  selectedVariation,
  ...
  const handleSave = (key: string) => {
    setDataSource(prev => prev.map(row => (row.key === key ? { ...row } : row)));
    setEditingKey(null);
  };
  ...
  const handleRemoveSelectedVariation = (record: JobVariationItems) => {
    setSelectedVariation(prev => prev.filter(item => item.key !== record.key));
  };
  const columns = [
    {
        ...
        isEditing(record) ? (
          <div className="flex flex-wrap gap-2">
            <Input
              ...
              onChange={e => handleChange(record.key, 'additional', e.target.value)}
            />
            <Select
      ...
      render: (_: any, record: JobVariationItems) =>
        isEditing(record) ? (
          <Switch
            ...
            onChange={checked => handleChange(record.key, 'drawingChanges', checked)}
          />
    ...
    },
    {
      ...
      render: (_: any, record: JobVariationItems) =>
        isEditing(record) ? (
          <span className="font-semibold">${record.total.toFixed(2)}</span>
        ) : (
        ...
        isEditing(record) ? (
          <Space>
            <Button
              ...
              shape="circle"
              icon={<IconCheck />}
              disabled={!record.additional.trim() || record.quantity <= 0 || record.price <= 0}
              onClick={() => handleSave(record.key)}
            />
            <Button danger shape="circle" icon={<IconX />} onClick={handleCancel} />
          </Space>
        ) : (
