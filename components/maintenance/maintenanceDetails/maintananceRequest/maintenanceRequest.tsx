...
const RequestList: React.FC = () => {
  ...
  const [openDescriptionNotes, setOpenDescriptionNotes] = useState<Record<string, Record<number, boolean>>>({});
  ...
  const handleDescriptionChange = (index: number, value: string) => {
    setNewRequestDescriptions(prev => 
  ...
  const reminderContent = (
    <div className="flex flex-col">
      ...
      <div className="flex justify-end mt-4">
        <Button key="ok" type="primary" onClick={handleReminderOk}>Ok</Button>
      </div>
    </div>
  );
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex gap-2 bg-white rounded-md p-3 text-sm font-medium mb-4 items-end">
        <div className="flex flex-col flex-1">
          <span className="mb-1 text-gray-600 font-semibold text-xs">
            Reference No
          </span>
          <Select
            placeholder="All"
        ...
        <div className="flex flex-col flex-1">
          ...
          <Input value={filters.supplier} onChange={(e) => handleFilterChange("supplier", e.target.value)} />
        </div>
        <div className="flex flex-col flex-1">
          <span className="mb-1 text-gray-600 font-semibold text-xs">Start</span>
          <Input value={filters.start} onChange={(e) => handleFilterChange("start", e.target.value)} />
        </div>
        <div className="flex flex-col flex-1">
          <span className="mb-1 text-gray-600 font-semibold text-xs">Finish</span>
          <Input value={filters.finish} onChange={(e) => handleFilterChange("finish", e.target.value)} />
        </div>
        <div className="flex flex-col flex-1">
          <span className="mb-1 text-gray-600 font-semibold text-xs">Complete</span>
          <Input value={filters.complete} onChange={(e) => handleFilterChange("complete", e.target.value)} />
        </div>
        <div className="flex flex-col flex-1">
        ...
        <div className="bg-white rounded-md border shadow-sm mb-4">
          ...
          <div className="p-4">
            ...
            {newRequestDescriptions.map((description, index) => (
              <div key={index} className="flex items-start text-sm gap-4 mb-2 pt-2 border-b border-gray-100 last:border-b-0">
