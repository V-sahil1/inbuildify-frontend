...
export const QuotationComparisionPdf = ({ comparisonResult, propertyAddress, selectedVersions, slugId }: Props) => {

    const v1 = selectedVersions?.[0] || { versionNumber: 'Version 1', totalAmount: '0.00' };
    const v2 = selectedVersions?.[1] || { versionNumber: 'Version 2', totalAmount: '0.00' };

    const Footer = () => (
        <View style={styles.footerWrapper} fixed>
            <Text style={styles.footerText} render={({ pageNumber, totalPages }) => `${pageNumber}/${totalPages}`} />
            <Text style={styles.footerLink}>https://app.insimplify.com.au/screens/LeadInfo.aspx?r=5594f23d-80bc-4da2-be62-13d96efd27c2</Text>
        </View>
    );
    ...
    const Header = () => (
        <View style={styles.headerContainer} fixed>
            <View style={styles.headerTopLine}>
                <Text style={styles.headerDate}>{new Date().toLocaleDateString('en-GB')}, {new Date().toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}</Text>
            ...
            <View style={styles.summaryBox}>
                <View style={styles.summaryRow}>
                    ...
                    <View style={styles.summaryColVersion}>
                        <Text style={styles.versionLabel}>Version {v1.versionNumber}</Text>
                        <Text style={styles.versionAmount}>${Number(v1.totalAmount).toFixed(2)}</Text>
                    </View>

                    {/* Column 3: Version 2 (Matches Value Column Flex 1) */}
                    <View style={styles.summaryColVersion}>
                        <Text style={styles.versionLabel}>Version {v2.versionNumber}</Text>
                        <Text style={styles.versionAmount}>${Number(v2.totalAmount).toFixed(2)}</Text>
                    </View>
                </View>
            </View>
        </View>
                    ...
                    {comparisonResult.map((item, index) => (
                        <View key={index} style={ItemTable.row} wrap={false}>
                            ...
                            <View style={ItemTable.colDescription}>
                                <Text style={ItemTable.categoryName}>{item.categoryName || 'Category'}</Text>
                                <Text style={ItemTable.itemDescription}>{item.description}</Text>
                            </View>

                            {/* Version 1 Value */}
                            <View style={ItemTable.colValue}>
                                <Text style={ItemTable.valueText}>{item.left?.value !== '-' ? item.left?.formattedValue : '-'}</Text>
                            ...
                            {/* Version 2 Value */}
                            <View style={ItemTable.colValue}>
                                <Text style={ItemTable.valueText}>{item.right?.value !== '-' ? item.right?.formattedValue : '-'}</Text>
