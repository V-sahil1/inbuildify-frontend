  ...
  return (
    <div className="bg-white rounded-lg shadow-sm text-gray-800">
      <p className="mb-4 text-base">
        The Job cannot be moved to <span className="font-bold">COMPLETED</span> as the below tasks
        are still <span className="font-bold">Pending / In Progress.</span>
      </p>

      {dummyPendingTasks.length > 0 && (
        <ul className="list-disc list-inside mb-6">
          {dummyPendingTasks.map((task, index) => (
            <li key={index} className="text-red-600 font-semibold flex items-center">
              <span className="inline-block h-2 w-2 mr-2 rounded-full bg-red-600"></span> {task}
            </li>
          ))}
        </ul>
      )}
      <p className="flex items-center gap-2">
        <IconAlertTriangle /> The Maintenance End Date is in the future. Completing the job now may
        affect warranty tracking.
      </p>
    </div>
  );
};
