...
export const getRanges = createAsyncThunk('floorPlans/ranges', async (_, { rejectWithValue }) => {
  try {
    const res = await api.get<ApiResponse<any>>(API_ENDPOINTS.RANGE);
    return res.data;
  } catch (error) {
...
export const updateRange = createAsyncThunk(
  ...
  async (payload: { id: string; name: string }, { rejectWithValue }) => {
    try {
      const res = await api.put<ApiResponse<any>>(API_ENDPOINTS.RANGE + '/' + payload.id, {
...
export const deleteRange = createAsyncThunk(
  ...
  async (payload: { id: string }, { rejectWithValue }) => {
    try {
      const res = await api.delete<ApiResponse<any>>(API_ENDPOINTS.RANGE + '/' + payload.id);
      return payload.id;
    } catch (error) {
      return rejectWithValue(error.message);
    }
...
export const getDwellingTypes = createAsyncThunk('floorPlans/dwellingTypes', async () => {
  try {
    const res = await api.get<ApiResponse<any>>(API_ENDPOINTS.DWELLING_TYPE);
    return res.data;
  } catch (error) {
...
export const createDwellingType = createAsyncThunk(
  ...
  async (payload: { name: string }, { rejectWithValue }) => {
    try {
      const res = await api.post<ApiResponse<DwellingType>>(API_ENDPOINTS.DWELLING_TYPE, {
        data: payload,
      });
      return res.data;
    } catch (error) {
...
export const updateDwellingType = createAsyncThunk(
  ...
  async (payload: { id: string; name: string }, { rejectWithValue }) => {
    try {
      const res = await api.put<ApiResponse<any>>(API_ENDPOINTS.DWELLING_TYPE + '/' + payload.id, {
        data: { name: payload.name },
      });
      return res.data;
    } catch (error) {
