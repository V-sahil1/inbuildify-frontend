...
export const JobCreationModal: React.FC<LeadCreationModalProps> = ({
  ...
  const { leadSources, status: leadStatus } = useAppSelector(
    (state) => state.lead
  );
  const { dwellingType, status: dwellingTypeStatus } = useAppSelector(
    (state) => state.types
  );
  ...
  useEffect(() => {
    if (open) {
      if (isEditing && initialValues) {
        ...
        if (valuesToSet.titleDate)
          valuesToSet.titleDate = dayjs(valuesToSet.titleDate);
        if (valuesToSet.quotationApprovedDate)
          valuesToSet.quotationApprovedDate = dayjs(
            valuesToSet.quotationApprovedDate
          );
        if (valuesToSet.quotationSignedDate)
          valuesToSet.quotationSignedDate = dayjs(
            valuesToSet.quotationSignedDate
          );
