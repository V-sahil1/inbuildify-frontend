...
import { IconDots, IconMail, IconMapPin, IconPencil, IconPhone, IconSearch, IconTable, IconTrash } from "@tabler/icons-react";
import { Button, Checkbox, Form, Image, Input, message, Radio, Select, Switch, Tag, Tooltip } from "antd";
...
const HLPackageDetail = () => {
    ...
    return (
        <div className="p-4">
            <Form form={form} onFinish={handleSubmit} initialValues={initialValues}>
                <div className="flex justify-between items-center mb-4">
                    ...
                    <div>
                        <div className="grid grid-cols-3 gap-8 text-primary">
                            <SelectEditableField open={RangeEditOpen} name='range' options={rangeOptions} onChange={() => {
                                setRangeEditOpen(false)
                            }} onClick={() => setRangeEditOpen(true)} />
                            <SelectEditableField open={DwellingTypeEditOpen} name='dwellingType' options={dwellingOptions} onChange={() => {
                                setDwellingTypeEditOpen(false)
                            }} onClick={() => setDwellingTypeEditOpen(true)} />
                            <SelectEditableField open={templateEditOpen} name='template' options={templateOptions} onChange={() => {
                    ...
                    <div className="text-primary border border-primary p-1 rounded-lg"> <Tooltip title='Go to Listing'><IconTable /></Tooltip></div>
                </div>
                <div>
                    <div className="grid grid-cols-2">
                        <div className="flex">
                            <div className="flex-1">
                                <ContentCard title="Title"><p className="p-4 text-sm">new</p></ContentCard>
                                <ContentCard title="Lot">
                                    <div className=" p-4 text-sm ">
                                        {lotDetail ?
                                            <div>
                                                <div className="flex gap-2 items-center mb-2"><IconMapPin size={20} /><p>{lotdata.address}</p></div>
                                                <div className="flex mb-2"> <p>Estate : </p><p>{lotdata.estate}</p></div>
                                                <div className="flex mb-2"> <p>Stage : </p><p>{lotdata.stage}</p></div>
                                                <div className="flex mb-2"> <p>Type : </p><p>{lotdata.type}</p></div>
                                                <div className="flex gap-1">
                                                    <div className="flex"> <p>W : </p><p>{lotdata.width}</p></div>
                                                    <div className="flex"> <p>D : </p><p>{lotdata.depth}</p></div>
                                                    <div className="flex"> <p>Total : </p><p>{lotdata.total}</p></div>
                                                </div>
                                            </div> :
                            ...
                            <div className="flex-1">
                                <ContentCard title="Contact">
                                    <div className="p-4 text-sm">
                                        <div className="mb-2">
                                            {contactNameEditOpen ? <Form.Item name='contactName'>
                                                <Select className="w-full" showSearch options={[{ label: 'Meet', value: 'Meet' }, { label: 'Rahul', value: 'Rahul' }]}
                                                    onChange={(value) => {
                                                        setContactNameEditOpen(false)
                                                    }} placeholder="Please Select Option"></Select>
                                            </Form.Item> : <p onClick={() => setContactNameEditOpen(true)}>{form.getFieldValue('contactName')}</p>}
                                        </div>
                                ...
                                <ContentCard title="Price">
                                    <div className="p-3 text-sm ">
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            <div className="flex gap-2" onClick={() => setLandCommissionModelOpen(true)}><p>Comm</p><p className="text-blue">$ 50000</p></div>
                                        </div>
                                    </div>
                                </ContentCard>
                            </div>
                                            ...
                                            <>
                                                <div className="h-full">
                                                    ...
                                                    <div className="flex justify-between items-center mt-[50px]">
                                                        ...
                                                        <IconTrash size={15} onClick={() => setSelectedPlan(null)} color="red" />
                                                    </div>
                                                </div>
                                            </>
                                        ) : (
                                            <div className="text-center mt-[100px] text-blue">
                                                Select Floor Plan
                                            </div>
                                        )}
                                    </div>
                                            ...
                                            <div className="flex items-center gap-2 mb-3">
                                                <div className="h-full flex flex-col justify-between">
                                                    ...
                                                    <div className="flex justify-between items-center mt-[55px]">
                                                        ...
                                                        <IconTrash size={15} onClick={() => setSelectedFacade(null)} color="red" />
                                                    </div>
                                                </div>
                                            </div>
                                        ) : (
                    ...
                    <div className="grid grid-cols-2">
                            ...
                            <div className="flex justify-between">
                                ...
                                <div className="rounded-2xl flex gap-2 p-1 border border-primary">
                                    {filterButtons.map((btn) => (
                                        <Button className={`rounded-xl text-xs ${activeTab === btn ? "bg-primary" : "bg-white text-primary"} `} type="primary" size="small" onClick={() => setActiveTab(btn)}>{btn}</Button>
                                    ))}
                                </div>
                                <div className="text-blue"><Tooltip title='Inclusion List'><IconTable size={20} /></Tooltip></div>
                            </div>
                            <div className="grid grid-cols-2 items-center gap-4 mt-3">
                                <Input addonBefore={<IconSearch size={20} />} placeholder="Search Inclusions" size="small" value={filters.inclusion}
                                    onChange={(e) =>
                                        handleFilterChange({ ...filters, inclusion: e.target.value })
                                    } />
                                <div>
                                    {groupEditOpen ? <Form.Item name='group' className="pt-3">
                                        <Select className="w-full" showSearch options={[{ label: 'Turnkey Inclusion', value: 'Turnkey Inclusion' }]}
                                            onChange={(value) => {
                                                setGroupEditOpen(false)
                                            }} placeholder="Please Select Option"></Select>
                                    </Form.Item> : form.getFieldValue('group') ? <div><p className="text-primary">{form.getFieldValue('group')}</p><p className="text-xs">Group</p></div> : <p className="text-blue" onClick={() => setGroupEditOpen(true)}>Choose Group</p>}
                                </div>
                            </div>
                            <div>
                        ...
                        <div className="flex-1">
                            <div className="p-4 m-2 bg-card-color">
                                ...
                                <Input addonBefore={<IconSearch size={20} />} placeholder="Search Feature.." size="small" value={filters.houseFeature}
                                    onChange={(e) =>
                                        handleFilterChange({ ...filters, houseFeature: e.target.value })
                                    } />
                                <div className="mt-2">
                            ...
                            <div className="p-4 m-2 bg-card-color">
                                ...
                                <TextArea rows={4} style={{ resize: 'none' }} showCount className="my-2" maxLength={500}></TextArea>
                            </div>
                        </div>
                    </div>
                </div>
                <div className="flex justify-between m-2">
                    <div className="flex gap-1">
                        ...
                        <Button type='primary' onClick={() => setCustomSectionModalOpen(true)}>Custom Section</Button>
                        <Button type='primary' onClick={() => setPriceListDrawerOpen(true)}>Price List</Button>
                    </div>
                    <h1 className="text-lg font-bold">Total : $53,000.00</h1>
                </div>
            </Form>

            <LandLotFormModel title='Lot' open={landLotOpen} onClose={() => setLandLotOpen(false)} onSubmit={() => {
            ...
            />
            <FloorPlanModal visible={floorplanModalOpen} onCancel={() => setfloorplanModalOpen(false)} onSave={setSelectedPlan} selectedPlan={selectedPlan} />
            <FacadeModal
            ...
            />
            <ActionDialogmodel title='Custom Section' open={customSectionModalOpen} onCancel={() => setCustomSectionModalOpen(false)} onSubmit={() => { }} fields={CustomSectionField} />
            <PriceListDrawer title='PriceList Items' open={PriceListDrawerOpen} onClose={() => setPriceListDrawerOpen(false)} />
        </div>
    )
}
