...
import { ContractTerms } from '../../pdf/ContractTerms';
...
const JobDocumentPdf = ({ showedSection }: jobDocument) => {
  const data = [
    {
      category: 'basePrice',
      items: [
    ...
    },
    {
      ...
      items: [
        ...
        { name: '2040 high internal doors', uom: '', units: '1', price: 'Package' },
        {
    ...
    },
    {
      category: 'internal',
      items: [
        { name: 'Flyscreen to all open-able windows', uom: '', units: '1', price: 'Package' },
      ],
    },
    {
      category: 'appliances',
...
900mm Electric Oven - Technika-T948SS-6`,
    ...
    {
      category: 'floorPlan',
      items: [{ name: 'Floor Plan', uom: '', units: '', price: 'Included' }],
    },
  ];
  ...
  const Footer = () => (
    <View style={styles.footerWrapper} fixed>
      <View style={{ flexDirection:'row',justifyContent:'space-between' }}>
        <Text style={styles.footerTopText}>01/12/2025</Text>
        <Text
          style={styles.footerTopText}
          render={({ pageNumber, totalPages }) => `Page ${pageNumber} of ${totalPages}`}
        />
        <Text style={styles.footerTopText}>INITIALS......./.......</Text>
      </View>
    </View>
  );
  ...
  return (
    <Document>
        ...
        <>
              ...
              <View>
                ...
                <View style={{ marginLeft: 5 }}>
                  <View style={[styles.secondaryText, { flexDirection: 'row', gap: 3 }]}>
                    <Text>1.</Text>
                    <Text>
                      Provide 2 sets of remote controls inclusive of drive motor and electrics to
                      the STD panel lift door
                    </Text>
                  </View>
                  <View style={[styles.secondaryText, { flexDirection: 'row', gap: 3 }]}>
                    <Text>2.</Text>
                    <Text>
                      Provide stainless steel trough with cupboard underneath up to one meter in
                      length
                    </Text>
                  </View>
                  <View style={[styles.secondaryText, { flexDirection: 'row', gap: 3 }]}>
                ...
                <View style={ItemTable.table}>
                  ...
                  {data.map((i, index) => (
                    <View key={index}>
                      <Text style={styles.subHeader}>{i.category}</Text>
                      {i.items.map((item, idx) => (
                        <Viewtabledata key={idx} item={item} />
                      ))}
                    </View>
                  ))}
          ...
          <PageLayout>
            <View>
              ...
              {data
                .filter(item => item.category === 'facade')
                .map((item, index) => (
                  <Image
                    key={`facade-${index}`}
              ...
              >
                ...
                <View style={styles.signatureTop}>
                  ...
                  >
                    ...
                    <Text style={styles.footerTopText}>John Wick</Text>
                  </View>
                  <View
                    style={{
                      flexDirection: 'column',
      ...
      {/* invoice */}
      {showedSection.invoice && (
          <Invoice />
      )}
      {/* invoice receipt */}
      {showedSection.invoiceReceipt && (
          <InvoiceReceipt />
      )}
      {/* color variation */}
      {showedSection.variation && (
          <ColorVariation />
      )}
    </Document>
  );
