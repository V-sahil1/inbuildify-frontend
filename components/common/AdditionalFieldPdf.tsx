...
const AdditionalFieldPdf = ({ }) => {
        ...
        RoofType: [
            {
                ...
                options: ['Concrete Tiles', 'Standard', 'Slimline', 'Flat', 'Colorbond Roof', 'With Blanket', 'With Sarking']
            },
            {
                label: 'Roof pitch',
                isCheckBox: true,
    ...
    const ViewData: React.FC<field> = ({ label, isCheckBox, Value, options }) => (
        <View style={styles.item}>
            ...
            <Text>: </Text>
            {isCheckBox ?
                <View style={{ width: 300, flexDirection: 'row', gap: 2, flexWrap: 'wrap' }}>: {options?.map((op) => (
                    Value === op ?
                        (<View style={{ flexDirection: 'row' }}> <View style={checkBoxStyle.container}><Image src='/check.png'></Image></View> <Text>{op}</Text> </View>)
                        : (<View style={{ flexDirection: 'row' }}> <View style={checkBoxStyle.container}></View> <Text>{op}</Text> </View>)
                ))}</View>
    ...
    return (
        <Document>
            <PageLayout>
                <View style={styles.titleText}><Text>Additions to New Job Form</Text></View>
                <View style={styles.headerText}><Text>Location Of Building Site</Text></View>
                {data.LocationOfBuildingSite.map((item) => (
                    <ViewData label={item.label} isCheckBox={item.isCheckBox} Value={item.Value} options={item.options} />
                ))}
                <View style={styles.headerText}><Text>Build Zone</Text></View>
                {data.BuildZone.map((item) => (
                    <ViewData label={item.label} isCheckBox={item.isCheckBox} Value={item.Value} options={item.options} />
                ))}
                <View style={styles.item}><Text style={styles.secondaryText}>Site Fill (Enter the values of maximum filled area)</Text></View>
                {data.SiteFill.map((item) => (
                    <ViewData label={item.label} isCheckBox={item.isCheckBox} Value={item.Value} options={item.options} />
                ))}
                <View style={styles.item}><Text style={styles.secondaryText}>Site Fall (As per Engineering)</Text></View>
                {data.SiteFall.map((item) => (
                    <ViewData label={item.label} isCheckBox={item.isCheckBox} Value={item.Value} options={item.options} />
                ))}
                <View style={styles.headerText}><Text>Design Guidelines and Other Requirements</Text></View>
                {data.DesignGuidelinesandOtherRequirements.map((item) => (
                    <ViewData label={item.label} isCheckBox={item.isCheckBox} Value={item.Value} options={item.options} />
                ))}
                <View style={styles.item}><Text style={styles.secondaryText}>Roof Type</Text></View>
                {data.RoofType.map((item) => (
                    <ViewData label={item.label} isCheckBox={item.isCheckBox} Value={item.Value} options={item.options} />
                ))}
                <View style={styles.item}><Text style={styles.secondaryText}>CONSTRUCTION TYPE</Text></View>
                {data.constructiontype.map((item) => (
                    <ViewData label={item.label} isCheckBox={item.isCheckBox} Value={item.Value} options={item.options} />
                ))}
                <View style={styles.item}><Text style={styles.secondaryText}>SETBACK FOR SINGLE STOREY AND GROUND FLOOR</Text></View>
                {data.setbackForSingleStoreyAndGroundFloor.map((item) => (
                    <ViewData label={item.label} isCheckBox={item.isCheckBox} Value={item.Value} options={item.options} />
                ))}
                <View style={styles.item}><Text style={styles.secondaryText}>SETBACK FOR DOUBLE STOREY FIRST FLOOR</Text></View>
                {data.setbackForDoubleStoreyFirstFloor.map((item) => (
                    <ViewData label={item.label} isCheckBox={item.isCheckBox} Value={item.Value} options={item.options} />
                ))}
                <View style={styles.item}><Text style={styles.secondaryText}>Additional Facade requests / Requirements</Text></View>
                <View>
                    <View style={{ fontSize: 10, marginBottom: 10, flexDirection: 'row' }}>
                        <View> <Text style={{ width: 200 }}>Facade Material Required</Text></View>
                            ...
                            <View style={{ width: 300 }}>
                                <View style={ItemTable.table}>
                                    ...
                                    <View style={[ItemTable.row, ItemTable.headerRow]}>
                                        <Text style={[ItemTable.cell, ItemTable.col15, { backgroundColor: '#1E3A8A', color: 'white' }]}>SNO</Text>
                                        <Text style={[ItemTable.cell, ItemTable.col40, { backgroundColor: '#1E3A8A', color: 'white' }]}>Material</Text>
                                        <Text style={[ItemTable.cell, ItemTable.col15, { backgroundColor: '#1E3A8A', color: 'white' }]}>Percentage</Text>
                                    </View>
                                    <View style={[ItemTable.row, ItemTable.headerRow]}>
                    ...
                    <View style={styles.item}>
                        <Text style={{ width: 200 }}>Parapet walls with box gutter behind
                            and pitched roof</Text>
                        <Text>: </Text>
                        <View style={{ flexDirection: 'row', gap: 2 }}>
                            <View style={{ flexDirection: 'row' }}><View style={checkBoxStyle.container}></View><Text>Yes</Text></View>
                            <View style={{ flexDirection: 'row' }}><View style={checkBoxStyle.container}><Image src='/check.png'></Image></View><Text>No</Text></View>
                        </View>
                    </View>
                    <View style={styles.item}>
                        <Text style={{ width: 200 }}>Parapet wall with box gutter and tray
                            deck roof</Text>
                        <Text>: </Text>
                        <View style={{ flexDirection: 'row', gap: 2 }}>
                            <View style={{ flexDirection: 'row' }}><View style={checkBoxStyle.container}></View><Text>Yes</Text></View>
                ...
                <View style={{ marginTop: 10 }}>
                    <View style={styles.item}>
                        <View style={checkBoxStyle.container}><Image src='/check.png'></Image></View>
                    ...
                    <View style={styles.item}>
                        <View style={checkBoxStyle.container}><Image src='/check.png'></Image></View>
                        <Text>DEVELOPER GUIDELINES (IF NOT FOUND ONLY YOU MAY NEED TO CONTACT DEVELOPERS
                            DIRECTLY)</Text>
                    </View>
                    <View style={styles.item}>
                        <View style={checkBoxStyle.container}><Image src='/check.png'></Image></View>
                        <Text>CONTACT FOR SALE (PLEASE NOTE MOST OF THE ABOVE ITEMS CAN BE FOUND IN THE
                            CONTRACT OF SALE)</Text>
                    </View>
                    <View style={styles.item}>
                        <View style={checkBoxStyle.container}></View>
                ...
                <View style={styles.headerText}><Text>Location Of Building Site</Text></View>
                <View style={styles.item}>
                    <Text>Lot has mild slope, retaining wall required at rear boundary. Ensure compliance with Wyndham Council guidelines.</Text></View>
            </PageLayout>
        </Document>
    )
};
...
const styles = StyleSheet.create({
    ...
    titleText: {
        alignItems: "center", fontWeight: "bold", marginBottom: 30, fontSize: 15
    },
    headerText: { backgroundColor: '#929292', fontSize: 10, color: 'white', padding: 6, marginBottom: 5, fontWeight: 'bold' },
