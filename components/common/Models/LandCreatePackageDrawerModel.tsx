...
const LandCreatePackageDrawerModel = ({ title, open, onClose, onSubmit }) => {
    ...
    return (
        <div>
            <Drawer title={title} open={open} onClose={onClose} closeIcon={false} extra={
                <IconX
                    ...
                    <Form form={form} layout="vertical" onFinish={handleSubmit}>
                        ...
                        <div className="grid grid-cols-2 gap-2 ">
                            <Form.Item label="Inclusion" name='inclusion' className="!text-red-500">
                                <Select options={[{label:'Turnkey Inclusion',value:'turnkeyInclusion'}]}></Select>
                            </Form.Item>
                        ...
                        <Form.Item label="Disclamier" name='disclamier'>
                            <Select options={[{label:'Validity',value:'validity'},{label:'Standard',value:'standard'}]}></Select>
                        </Form.Item>
                        {/* floor plan */}
                        <div>
                            <p>Floor Plan</p>
                            <Form.Item name='floorplan'>
                                <div className="flex gap-2 overflow-x-auto mt-3">
                                    {floorPlans?.map((plan) => (
                                        <div className={`${selectedFloorplan === plan.floorPlanId ? 'border-2 border-primary' : ''}`}>
                                            <FloorPlanCard
                        ...
                        <div className="mt-3">
                            <div className="flex justify-between">
                                ...
                                <div className="flex gap-4">
                                    <Checkbox checked={true} onChange={(e) => { }}>Standard </Checkbox>
                                    <Checkbox checked={false} onChange={(e) => { }}>Upgrade </Checkbox>
                                </div>
                            </div>
                            <Form.Item name='facade'>
                                <div className="flex gap-2 overflow-x-auto mt-3">
                                    ...
                                    {facades?.map((facade) => (
                                        <div key={facade.facadeId} className={`h-full ${selectedFacade === facade.facadeId ? 'border-2 border-primary' : " "}`}>
                                            <Card
                                                hoverable
                                                onClick={() => {
