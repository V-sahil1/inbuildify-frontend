...
const CampaignDetail = ({ current, setCurrent }) => {
    ...
    return (<div>
            ...
            <div className="flex-col flex gap-3">
                <div className="flex">
                    ...
                    <div className="w-[500px]">
                        <Button className={` ${activeTab === "Email" ? "bg-primary" : "bg-white text-primary"} rounded-none`} type="primary" onClick={() => setActiveTab("Email")}>Email</Button>
                        <Button className={` ${activeTab === "SMS" ? "bg-primary" : "bg-white text-primary"} rounded-none`} type="primary" onClick={() => setActiveTab("SMS")}>SMS</Button></div>
                </div>
                <div className="flex">
                ...
                <div className="flex">
                    ...
                    <Input className=" w-[500px]" addonAfter={<Dropdown menu={{
                        items: [
                            { key: 'Insert Contact Name', label: 'Insert Contact Name', onClick: () => { } },
                        ]
                    }}>Insert Personalization</Dropdown>} />
                </div>
                <div className="flex">
        ...
        <div className="flex justify-between mx-3">
            <p>Campaign Footer</p>
            <div><p className="text-blue">Choose Footer</p><p>Standard Footer</p></div>
            <p className="text-blue" onClick={() => setFooterOpen(true)}>Create Footer</p>
        </div>
        <CreateFormModal title="Footer" open={footerOpen} onCancel={() => setFooterOpen(false)} onSubmit={(values) => {
            console.log(values)
        }} fields={fields} initialValues={initialValues} isEditing={false} />
    </div>)
}
