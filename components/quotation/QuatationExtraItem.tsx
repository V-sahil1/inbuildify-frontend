...
export const QuatationExtraItem: React.FC<QuatationItemProps> = React.memo(
    ({ onQuantityChange, onToggleAdd ,onItemQuantityChange,
        ...
        const { extraItems } = useAppSelector(
        ...
        const handleToggle = async () => {
            ...
            //   onToggleAdd();
            try {
                if(formData.category_id === undefined || formData.cost_type === undefined || formData.description === "" || formData.quantity === 1 || formData.cost === 0){
                    message.error("Please fill all the fields");
                    return;
                }
                if (newAdded) {
                    const response = await dispatch(createCategoryItem({
                        ...
                        <div className="flex flex-col flex-wrap gap-5 mt-1">
                            <div className="flex gap-5">
                                ...
                                <Select
                                    ...
                                    onChange={(val) =>
                                        setFormData((prev) => ({ ...prev, cost_type: val }))
                                    }
                                    style={{ minWidth: "180px" }}
                                >
