...
import { IconBattery, IconBellOff, IconBook2, IconDotsVertical, IconMail, IconSearch, IconShare3, IconSpeakerphone, IconUsersGroup } from "@tabler/icons-react"
        ...
        },
        {
            ...
            render: () => (
                <div className="flex gap-2 justify-end">
                    <div><IconDotsVertical size={15} /></div>
                    <div><IconShare3 size={15} /></div>
                </div>
            )
        },
            ...
            <div className="flex justify-between gap-2 mt-3">
                {card.map((op) => (
                    <div className="p-3 bg-card-color w-[200px]">
                        ...
                        {op.footer && (<><Divider className="!mb-2"></Divider><p className="text-xs">{op?.footer}</p></>)}
                    </div>
                ))}
            </div>
            <div className="mt-6">
                <div className="flex justify-between mb-2 ">
                    <div className="flex gap-2 w-[60%]"><Input addonBefore={<IconSearch size={20} />} value={campaignName}
                        onChange={(e) =>
                            setCampaignName(e.target.value)
                        } placeholder="Search Campaigns by Campaign name" style={{ width: "80%" }} /><p className="text-sm">54 Campaigns</p></div>
                    <Button type="primary" onClick={() => router.push('/campaigns/CreateCampaign')} >New Campaign</Button>
                </div>
