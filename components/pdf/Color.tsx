import { Text, View, StyleSheet, Image, Page, Document } from '@react-pdf/renderer';
export const Signature = ({ items }) => (
  <View>
    ...
    <View style={styles.signatureTop}>
      ...
      >
        <Text style={styles.footerTopText}>{items.label}</Text>
      </View>
      <View
        style={{
          flexDirection: 'column',
      ...
      >
        ...
        <Text style={styles.footerTopText}>{items.name}</Text>
      </View>
      <View
        style={{
          flexDirection: 'column',
      ...
      >
        <Text style={styles.footerTopText}>___________________________</Text>
        <Text style={styles.footerTopText}>{items.date}</Text>
      </View>
    </View>
  </View>
);
export const Color = ({ colorData }) => {
  ...
  return (
    ...
    <PageLayout>
              ...
              {categoryGroup.items.map((item, i) => (
                <View key={`${index}-${i}`} style={ItemTable.row}>
                  ...
                  <Text style={[ItemTable.cell, ItemTable.col40]}>
                    {item.color}
                    {'\n'}
                    <Text style={{ fontSize: 8, color: '#999' }}>{item.status || 'Standard'}</Text>
                  </Text>
      ...
      <Signature items={{ label: "Owner's signature", name: 'Name of Lead', date: 'Date ' }} />
    </PageLayout>
    // </Document>
  );
};
