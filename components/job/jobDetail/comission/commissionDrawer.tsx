...
export const CommissionDrawer = ({
  ...
  const saveEditedPartner = useCallback(() => {
    ...
    setPartners(prev =>
      prev.map(p =>
        p.id === editingPartnerId ? { ...p, name: editPartnerName, type: editRecipientType } : p
      )
    );
  ...
  const handleStageSave = useCallback(
    (partnerId: string, stageIdx: number) => {
      if (
        !editingStage ||
        editingStage.partnerId !== partnerId ||
        editingStage.stageIdx !== stageIdx
      )
        return;
            ...
            ? {
                ...
                stages: p.stages.map((s, i) =>
                  i === stageIdx
                    ? {
                        ...s,
                        name: editingStage.newName,
                        total: editingStage.newTotal,
                        toBePaid: Math.max(editingStage.newTotal - s.paid, 0),
                      }
                    : s
                ),
              }
  ...
  const handleDeleteStage = useCallback(
    (partnerId: string, stageIdx: number) => {
      setPartners(prevPartners =>
        prevPartners.map(p =>
          p.id === partnerId ? { ...p, stages: p.stages.filter((_, i) => i !== stageIdx) } : p
        )
      );
    },
    [setPartners]
  ...
  const handleNewStageSave = useCallback(
    (partnerId: string, nextStageSortOrder: number) => {
      ...
      setPartners(prevPartners =>
        prevPartners.map(p => (p.id === partnerId ? { ...p, stages: [...p.stages, newStage] } : p))
      );
      cancelNewStage();
    },
  ...
  const togglePartnerEnabled = (partnerId: string, checked: boolean) => {
    setDisabledPartners(prev =>
      checked ? prev.filter(id => id !== partnerId) : [...prev, partnerId]
    );
  };

  const NewPartnerRow = useMemo(
    () =>
      isCreatingNew && (
        <div className="grid grid-cols-2 lg:grid-cols-5 items-center p-2 bg-blue-50 border-b border-gray-300">
          ...
          <div className="hidden lg:block lg:col-span-1">
            <Select
              ...
              value={newRecipientType || undefined}
              onChange={setNewRecipientType}
          ...
          <div className="col-span-1 flex justify-end gap-2">
            <button
              ...
              onClick={handleNewPartnerSave}
              disabled={!newPartnerName || !newRecipientType}
            >
              <IconCheck size={18} />
            </button>
            <button
  ...
  const PartnerRow = useCallback(
    (partner: Partner) => {
      ...
      const EditingRow = (
        <div className="grid grid-cols-2 lg:grid-cols-5 items-center p-2 bg-white border-y border-gray-200">
          ...
          <div className="hidden lg:block lg:col-span-1">
            <Select
              placeholder="Please select"
              value={editRecipientType || undefined}
              onChange={value => setEditRecipientType(value as Partner['type'])}
              className="w-full"
            >
          ...
          <div className="col-span-1 flex justify-end gap-2">
            <button
              className="text-green-600 hover:text-green-800"
              onClick={saveEditedPartner}
              disabled={!editPartnerName || !editRecipientType}
            >
              <IconCheck size={18} />
            </button>
      ...
      const DisplayRow = (
        <div className="grid grid-cols-2 lg:grid-cols-5 items-center p-2 bg-white font-semibold flex-wrap">
          <div className="flex items-center gap-2 col-span-2 lg:col-span-1">
            <Switch
              checked={isEnabled}
              onChange={checked => togglePartnerEnabled(partner.id, checked)}
            />
          ...
          <div className="col-span-1 flex justify-end gap-2">
            <button
              className="text-green-600 hover:text-green-800"
              disabled={!isEnabled || isAddingNewStage}
              onClick={() => startAddingNewStage(partner.id)}
            >
              <IconPlus size={18} />
            </button>
            <button
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 items-center p-2 bg-gray-50 border-t border-b border-gray-300">
          ...
          <div className="col-span-1 flex items-center gap-2">
            <Input
              ...
              placeholder="Value"
              value={newStageValue}
              onChange={e => setNewStageValue(Number(e.target.value))}
              className="w-full"
            />
          </div>
          <div className="hidden lg:block lg:col-span-1 ml-12">
          ...
          <div className="col-span-1 flex justify-end gap-2">
            <button
              className="text-green-600 hover:text-green-800"
              onClick={() => handleNewStageSave(partner.id, nextStageSortOrder)}
              disabled={!newStageName || newStageValue <= 0}
            >
              <IconCheck size={18} />
            </button>
      ...
      const StageRows =
        ...
        partner.stages.map((stage, idx) => {
          ...
          const isEditingThisStage =
            editingStage?.partnerId === partner.id && editingStage.stageIdx === idx;
              ...
              <div className="col-span-1 flex items-center">
                ...
                {isEditingThisStage ? (
                  <Input
                    ...
                    onChange={e =>
                      setEditingStage(prev => (prev ? { ...prev, newName: e.target.value } : null))
                    }
                    className="w-full"
                  />
                ) : (
              ...
              <div className="col-span-1 flex items-center gap-2">
                {isEditingThisStage ? (
                  <Input
                    type="number"
                    prefix="$"
                    value={editingStage!.newTotal}
                    onChange={e =>
                      setEditingStage(prev =>
                        prev ? { ...prev, newTotal: Number(e.target.value) } : null
                      )
                    }
                  ...
                  <>
                    <span className="lg:hidden">Value:</span> ${stage.total.toLocaleString()}{' '}
                    {percent > 0 && <Tag color="green">{percent}%</Tag>}
                  </>
                )}
              </div>
              ...
              <div className="col-span-1 flex justify-end gap-2">
                {isEditingThisStage ? (
                  <>
                    <button
                      className="text-green-600 hover:text-green-800"
                      onClick={() => handleStageSave(partner.id, idx)}
                    >
                      <IconCheck size={18} />
                    </button>
                    <button
                  ...
                  <>
                    <button
                      ...
                      onClick={() => startEditingStage(partner, stage, idx)}
                    >
                      <IconEdit size={18} />
                    </button>
                    <Popconfirm
                      title={`Are you sure you want to delete ${stage.name}?`}
                      onConfirm={() => handleDeleteStage(partner.id, idx)}
                      okText="Yes, Delete"
                      cancelText="No"
                    >
    ...
    [
      ...
      editRecipientType,
      editCommissionValue,
      addingNewStageForPartnerId,
      newStageName,
      newStageValue,
      editingStage,
      getStagePercent,
      cancelPartnerEditing,
      saveEditedPartner,
      startAddingNewStage,
      startEditingPartner,
