import { IconChevronDown, IconCurrencyDollar, IconPencil, IconSettings, IconCheck } from "@tabler/icons-react";
    ...
    const initialValue = [
        ...
        },
        {
            ...
            option: [
                {
                    label: 'Yes',
                    onclick: () => { /* mark as included */ },
                    color: 'bg-green-700'
                },
                {
                    label: 'No',
                    onclick: () => { /* handle exclusion */ },
                    color: 'bg-blue'
                }
            ],
            ...
            option: [
                {
                    ...
                    onclick: () => { /* create extension notice */ },
                    color: 'bg-blue'
                },
                {
                    label: 'Send via Email',
    ...
    return (
        <div className="p-3">
            <div className="flex justify-between border-b-2 p-3 ">
                <div className="flex gap-2" >{data[0].id}<IconPencil size={20} /></div>
            ...
            <div className="mx-100 mt-20">
                ...
                <div className="flex justify-between">
                    <div> <Tag className={`${status === 'Approved' ? 'bg-green-500 text-white' : 'bg-gray-300'}`} >{status}</Tag></div>
                    <div className="text-lg font-extrabold flex"><IconCurrencyDollar />{data[0].amount}</div>
                </div>
                <div className="relative">
                    <div style={{ zIndex: 2 }} className="absolute top-0 bottom-0 border border-gray-400 w-0 left-9"></div>

                    <div className="flex flex-col mt-20 gap-4" >
                        
                        {step.map((item, index) => {
                           
                            return <div style={{ zIndex: 3 }} className={`flex justify-between border ${(activeIndex == index) ? 'border-primary' : 'border-border-color'} bg-card-color p-20  shadow-lg`}>

                                <div className="flex gap-6 ">
                                    <div className={`${(item.visible) ? 'text-font-color border-primary' : 'text-font-color-400 border-border-color'} 
                                    ${item.state ? 'bg-green-700 border-none text-white' : ''} flex items-center justify-center border  rounded-full w-8 h-8`} >
                                        {item.state ? <IconCheck /> : index + 1}
                                    </div>
                                    <div>
                                        <div className={`content-start ${(item.visible) ? 'text-font-color border-primary' : 'text-font-color-400 border-border-color'} `}>{item.title}</div>
                                        {item.visible && <div className='text-blue text-sm' >{item?.text}</div>}

                                    </div>
                                </div>
                                {(item.visible) &&
                                    <div className="flex gap-1">
                                        {item?.option.map((op, ind) => (
                                            <Button className={`border-none text-white ${op?.color}`} onClick={() => {
